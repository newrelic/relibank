...
| `BASE_URL` | manual (e.g. `http://sandbox.relibankdemo.com`) | Public app URL the loadgen/scenario-trigger `flow-*.yml` workflows hit (frontend, `support-service/chat`, `scenario-runner`, etc.). Not used by the deployer itself — consumed only by the loadgen flows listed in [runbook.md](runbook.md). Also read by [`relibank-sourcemaps.yml`](../.github/workflows/relibank-sourcemaps.yml), which fetches the live Vite manifest/sourcemaps from this host. | Loadgen flows 404/time out against the wrong host, or (worse) generate real traffic against another environment. For the sourcemaps workflow: manifest fetch 404s/returns HTML instead of JSON if this doesn't match the env's actually-deployed host |
...
| `NR_LICENSE_KEY` | NR UI → API keys → Ingest – License | **APM** ingest. Server-side agents send traces/metrics with this. Also injected into the cluster-side `nri-bundle` / `nr-ebpf-agent` helm charts installed by `relibank-newrelic.yml` — this same `nri-bundle` release is what runs the `nri-postgresql` classic on-host integration (see `nr_infra_agent.tf`) — and (as of the mssql/kafka collector port) into `app_module`'s `nrdot-collector-mssql` / `otel-collector-kafka` Deployments via `deploy-relibank.yml`'s existing `-var="new_relic_license_key=..."` on `relibank-{color}` → `app_module`. Format: `<32 hex chars>FFFFNRAL`. | No APM data in NR for any service; no cluster-side infrastructure/Kubernetes/logging/prometheus telemetry either (including `nri-postgresql`'s `PostgresqlDatabaseSample`/etc.); mssql/kafka collector exports 401 with a bad key |
...
| `POSTGRES_USER` | manual (typically `postgres`) | Postgres admin username. Also passed into `relibank-newrelic.yml` → `nr_infra_agent.tf` to authenticate the `nri-postgresql` on-host integration (reuses the app tier's superuser credential, no dedicated monitoring role). | accounts-service / auth-service connections fail; `nri-postgresql` auth fails too, so `PostgresqlDatabaseSample`/`PostgresSlowQueries`/`PostgresIndividualQueries` stop reporting |
| `POSTGRES_PASSWORD` | manual | Postgres admin password. Same dual use as `POSTGRES_USER` above. | Same as above |
...
| [`deploy-relibank.yml`](../.github/workflows/deploy-relibank.yml) | All Azure secrets, MSSQL/Postgres secrets, `NR_LICENSE_KEY`, `DNS_ZONE`, AKS vars, TF state vars |
...
| [`relibank-sourcemaps.yml`](../.github/workflows/relibank-sourcemaps.yml) | `NR_USER_API_KEY`, `NR_BROWSER_APP_ID`, `BASE_URL` |
