...
| `NR_LICENSE_KEY` | NR UI → API keys → Ingest – License | **APM** ingest. Server-side agents send traces/metrics with this. Also injected into the cluster-side `nri-bundle` / `nr-ebpf-agent` helm charts installed by `relibank-newrelic.yml`, and (as of the mssql/kafka collector port) into `app_module`'s `nrdot-collector-mssql` / `otel-collector-kafka` Deployments via `deploy-relibank.yml`'s existing `-var="new_relic_license_key=..."` on `relibank-{color}` → `app_module`. Format: `<32 hex chars>FFFFNRAL`. | No APM data in NR for any service; no cluster-side infrastructure/Kubernetes/logging/prometheus telemetry either; mssql/kafka collector exports 401 with a bad key |
