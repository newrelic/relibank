...
def fetch_manifest(public_host: str) -> dict:
    ...
    if resp.status_code != 200:
        print(f"Error: manifest not reachable at {url} ({resp.status_code}). "
              f"Is {public_host} deployed with `build.manifest: true`?", file=sys.stderr)
        sys.exit(1)
    try:
        return resp.json()
    except ValueError:
        print(f"Error: {url} did not return valid JSON. First 300 chars:\n{resp.text[:300]}",
              file=sys.stderr)
        sys.exit(1)
...
def main() -> int:
    ...
    if failed:
        return 1
    if uploaded == 0 and already_current == 0:
        print("Error: no sourcemaps were uploaded or already current — treating as failure.",
              file=sys.stderr)
        return 1
    return 0
