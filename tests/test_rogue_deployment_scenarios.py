...
def reset_risk_scenarios():
    ...
    # Reset before test
    response = requests.post(f"{SCENARIO_SERVICE_URL}/api/risk-assessment/reset", timeout=10)
    ...
    # Cleanup after test
    try:
        requests.post(f"{SCENARIO_SERVICE_URL}/api/risk-assessment/reset", timeout=10)
    except:
...
def wait_for_agent_config(expected_agent: str, expected_rogue_enabled: bool, timeout: int = 10) -> bool:
    ...
    while time.time() - start_time < timeout:
        response = requests.get(f"{SCENARIO_SERVICE_URL}/api/risk-assessment/config", timeout=5)
...
def enable_rogue_agent() -> Dict:
    ...
    response = requests.post(
        f"{SCENARIO_SERVICE_URL}/api/risk-assessment/rogue-agent",
        params={"enabled": True},
        timeout=10
    )
    assert response.status_code == 200
...
def test_get_risk_assessment_config(reset_risk_scenarios):
    ...
    response = requests.get(f"{SCENARIO_SERVICE_URL}/api/risk-assessment/config", timeout=10)
    assert response.status_code == 200
