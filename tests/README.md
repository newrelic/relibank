...
- **Local**:
  ...
  - Scenario Service: http://localhost:8000/scenario-runner
...
Set environment variables to test against a remote deployment:

```bash
# Export environment variables for all services
export BASE_URL="https://your-server.example.com"
export ACCOUNTS_SERVICE="https://your-server.example.com/accounts"
export BILL_PAY_SERVICE="https://your-server.example.com/billpay"
export CHATBOT_SERVICE="https://your-server.example.com/chatbot"
...
Or as a one-liner for end-to-end tests:
```bash
BASE_URL="https://your-server.example.com" ACCOUNTS_SERVICE="https://your-server.example.com/accounts" BILL_PAY_SERVICE="https://your-server.example.com/billpay" CHATBOT_SERVICE="https://your-server.example.com/chatbot" pytest tests/test_end_to_end.py -v -s
```
...
| `SCENARIO_SERVICE_URL` | Scenario service API URL | `http://localhost:8000/scenario-runner` |
...
- User accounts created for testing are logged out
- Payment scenarios are reset before and after tests
- Chaos/locust smoke tests clean up after enabling scenarios

Manual cleanup if tests are interrupted:
```bash
# Reset all scenarios
curl -X POST http://localhost:8000/scenario-runner/api/payment-scenarios/reset
curl -X POST http://localhost:8000/scenario-runner/api/chaos-scenarios/reset

# Stop Locust
curl -X POST http://localhost:8000/scenario-runner/api/locust-scenarios/stop
```
