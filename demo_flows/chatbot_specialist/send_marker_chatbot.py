...
# Usage: python send_marker_chatbot.py badCommit <user_key> <account_id> <env>
...
if __name__ == '__main__':
    print("Chatbot Specialist Deployment Marker")
    ...
    if marker_type == "badCommit":
        print("Sending deployment marker - Updating coordinator agent")
        category = "DEPLOYMENT"
        type = "ROLLING"
        version = "v2.1.0"
        user = "devops_automation"
        short_desc = "Coordinator agent update"
        desc = "Deploying updated coordinator agent - optimizing specialist delegation"
        group_id = "CHATBOT-SPEC-2024"
        # Use APP_NAME from Dockerfile: "${APP_NAME} - Chatbot Service"
        guid = getEntityGuid(user_key, "Chatbot Service", environment)
        sendMarkerEvent(category, type, commit, version, user, short_desc, desc, group_id, environment, guid)

    elif marker_type == "goodCommit":
        print("Sending rollback marker - Reverting coordinator changes")
        category = "DEPLOYMENT"
        type = "BASIC"
        version = "v2.0.5"
        user = "devops_automation"
        short_desc = "Rollback coordinator agent"
        desc = "Reverting coordinator agent to previous stable version"
        group_id = "CHATBOT-SPEC-2024"
        guid = getEntityGuid(user_key, "Chatbot Service", environment)
        sendMarkerEvent(category, type, commit, version, user, short_desc, desc, group_id, environment, guid)
